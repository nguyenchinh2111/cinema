...
interface DialogUpdateRoomProps {
  room: Room;
  trigger?: React.ReactNode;
  onRoomUpdated?: (room: RoomFormData) => void;
}

export function DialogUpdateRoom({ 
  room, 
  trigger, 
  onRoomUpdated 
}: DialogUpdateRoomProps) {
  ...
  // Pre-fill form with room data when dialog opens
  useEffect(() => {
    if (isOpen && room) {
      ...
      setValue("screenSize", room.screenSize);
      setValue("soundSystem", room.soundSystem);
      setValue("features", Array.isArray(room.features) ? room.features.join(", ") : room.features);
  ...
  const onSubmit: SubmitHandler<RoomFormData> = async (data) => {
    ...
    try {
      ...
      console.log("Updating room:", { id: room.id, ...data });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white border-gray-300">
        <DialogHeader>
          <DialogTitle className="text-[#1E201E]">Update Room</DialogTitle>
        </DialogHeader>
