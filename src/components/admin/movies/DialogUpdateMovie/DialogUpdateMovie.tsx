...
interface DialogUpdateMovieProps {
  movie: Movie;
  trigger?: React.ReactNode;
  onMovieUpdated?: (movie: MovieFormData) => void;
}

export function DialogUpdateMovie({
  movie,
  trigger,
  onMovieUpdated,
}: DialogUpdateMovieProps) {
  ...
  // Pre-fill form with movie data when dialog opens
  useEffect(() => {
    if (isOpen && movie) {
  ...
  const onSubmit: SubmitHandler<MovieFormData> = async (data) => {
    ...
    try {
      ...
      console.log("Updating movie:", { id: movie.id, ...data });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white border-gray-300">
        <DialogHeader>
          <DialogTitle className="text-[#1E201E]">Update Movie</DialogTitle>
        </DialogHeader>
